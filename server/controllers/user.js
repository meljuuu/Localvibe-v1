...
exports.resendVerificationEmail = catchAsyncErrors(async (req, res, next) => {
  try {
    console.log("Received request to resend verification email"); // Log the request initiation

    const { email } = req.body;
    console.log("Email provided:", email); // Log the email provided in the request
    ...
    if (user.isVerified) {
      console.log("Email already verified for user:", user.email); // Log if the email is already verified
    ...
    await user.save();
    console.log("User updated with new verification token and expiration time"); // Log after user is updated

    // Send the verification email
    await sendVerificationEmail(user.email, verificationToken);
    console.log("Verification email sent to:", user.email); // Log after sending the email
    ...
    console.error("Error in resendVerificationEmail:", error); // Log the error
    res.status(500).json({
      success: false,
      message: error.message,
    });
